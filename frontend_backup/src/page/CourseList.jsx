    ...
    >
      <div className="flex-grow">
        <div className="flex items-center gap-2 mb-1">
          <span
            className={`px-2 py-0.5 rounded-lg text-[10px] font-bold ${theme.badge}`}
          >
        ...
        {totalCount > visibleCount && (
          <div className="flex justify-center my-4">
            <button
              ...
              disabled={isLoading}
              className={`btn ${
                isCompulsory ? "btn-primary" : "btn-warning"
              } btn-wide rounded-2xl shadow-lg border-none text-white ${
                isLoading ? "loading" : ""
              }`}
            >
              {isLoading
                ? "讀取中..."
                : `載入更多 (${visibleCount}/${totalCount})`}
            </button>
          </div>
        )}
...
const CourseList = () => {
  ...
  const {
    ...
    const checkFilterMatch = (c) => {
      ...
      const matchCategory =
        filters.category === "" ||
        (c.category ?? "").trim() === filters.category;
      const matchCredits =
        filters.credits === "" || String(c.credits ?? "") === filters.credits;
      const matchDay =
        filters.day === "" ||
        (c.schedule?.length > 0 && String(c.schedule[0].day) === filters.day);
      const matchLocation =
        filters.location === "" ||
        (c.schedule?.length > 0 &&
          (c.schedule[0].location ?? "").includes(filters.location));
      const matchSessionId =
        filters.session_id === "" ||
        (c.session_id ?? "").includes(filters.session_id);
    ...
    return {
      displayCompulsory: compulsory.slice(0, visibleCompulsory), //  各自獨立切片
  ...
  //  各自獨立的載入更多處理函數
  const handleLoadMoreCompulsory = () => {
    setIsLoadingCompulsory(true);
    setTimeout(() => {
      setVisibleCompulsory((prev) => prev + BATCH_SIZE);
      setIsLoadingCompulsory(false);
    }, 400);
  };

  const handleLoadMoreElective = () => {
    setIsLoadingElective(true);
    setTimeout(() => {
      setVisibleElective((prev) => prev + BATCH_SIZE);
      setIsLoadingElective(false);
    }, 400);
  };

  const handleLoadMoreChinese = () => {
    setIsLoadingChinese(true);
    setTimeout(() => {
      setVisibleChinese((prev) => prev + BATCH_SIZE);
      setIsLoadingChinese(false);
    }, 400);
  };

  const handleLoadMoreEnglish = () => {
    setIsLoadingEnglish(true);
    setTimeout(() => {
      setVisibleEnglish((prev) => prev + BATCH_SIZE);
      setIsLoadingEnglish(false);
    }, 400);
  };

  const handleLoadMorePe = () => {
    setIsLoadingPe(true);
    setTimeout(() => {
      setVisiblePe((prev) => prev + BATCH_SIZE);
  ...
  const handleAddCourse = (course) => {
    ...
    if (conflictCourse) {
      alert(
        `⚠️ 時間衝突！\n「${course.title}」與已選的「${conflictCourse.title}」衝堂`,
      );
      return;
    }
    // 通過邏輯檢查課程重複問題
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex relative items-center gap-1">
            ...
            {isFilterOpen && (
              ...
              >
                <p className="text-sm text-center font-bold text-gray-700 mb-1">
                  進階篩選
                </p>
                ...
                <div className="form-control w-full">
                  ...
                  <select
                    className="select select-bordered select-sm w-full bg-white/50"
                ...
                <div className="form-control w-full">
                  ...
                  <input
                    type="text"
                    placeholder="輸入教授姓名..."
                    className="input input-bordered input-sm w-full bg-white/50"
      ...
      <div className="py-2">
        {totalFilteredCount > 0 ? (
          <>
            ...
            <CourseSection
              title="必修課程"
              count={compulsoryTotal}
              courses={displayCompulsory}
              icon="📖"
              defaultOpen={false}
              isCompulsory={false}
              visibleCount={visibleCompulsory}
              totalCount={compulsoryTotal}
              onLoadMore={handleLoadMoreCompulsory}
              isLoading={isLoadingCompulsory}
              onAddCourse={handleAddCourse}
            />

            {/* 選修區塊，傳入獨立的 visibleCount 與 onLoadMore */}
            <CourseSection
              title="選修課程"
              count={electiveTotal}
              courses={displayElective}
              icon="🏛️"
              defaultOpen={false}
              isCompulsory={false}
              visibleCount={visibleElective}
              totalCount={electiveTotal}
              onLoadMore={handleLoadMoreElective}
            ...
            <CourseSection
              title="國文課程"
              count={chineseTotal}
              courses={displayChinese}
              icon="✍️" // 給個專屬圖示
              defaultOpen={false} // 預設先收合，避免畫面一開始太長
              isCompulsory={true} // 這裡設為 true 可以讓按鈕套用你寫好的藍色樣式
              visibleCount={visibleChinese}
              totalCount={chineseTotal}
              onLoadMore={handleLoadMoreChinese}
              isLoading={isLoadingChinese}
              onAddCourse={handleAddCourse}
            />
            ...
            <CourseSection
              title="英文課程"
              count={englishTotal}
              courses={displayEnglish}
              icon="🔤" // 給個專屬圖示
              defaultOpen={false} // 預設先收合，避免畫面一開始太長
              isCompulsory={true} // 這裡設為 true 可以讓按鈕套用你寫好的藍色樣式
              visibleCount={visibleEnglish}
              totalCount={englishTotal}
              onLoadMore={handleLoadMoreEnglish}
            ...
            <CourseSection
              ...
              defaultOpen={false}
              isCompulsory={false} //
              visibleCount={visiblePe}
              totalCount={peTotal}
              onLoadMore={handleLoadMorePe}
